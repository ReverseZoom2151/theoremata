...
pub fn poll_with_provider(
    ...
    let verification = if lean_code.is_empty() {
        ...
        verify_output(store, config, &job, &lean_code).ok()
    };
    ...
    // Mirror of the aristotle site: the completion branch, where the store, the
    // result and the graph coordinates are all in hand. Written before the
    // result is moved into the job.
...
///
/// Called from the completion branch of `poll_with_provider`, the mirror of the
/// aristotle site: that is the only point where the store, the result and the
/// graph coordinates are all in hand at once.
///
...
fn record_artifact_evidence(store: &Store, job: &ProofJob, result: &ProofResult) -> Result<()> {
    ...
    store.add_evidence(
        ...
        // the proof. The run's own claimed status goes in the payload, where it
        // cannot be misread as a gate this row never ran.
        "recorded",
...
mod tests {
    ...
    fn job_with(store: &Store, ids: (Option<String>, Option<String>)) -> ProofJob {
        ...
        store
            .create_proof_job(&task, BACKEND, ProverJobStatus::Queued, None, None, 0.0)
            .unwrap()
    }
